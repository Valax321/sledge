...
namespace Sledge.BspEditor.Environment.Source
{
    public class SourceEnvironment : IEnvironment
    {
        ...
        {
            return new SourceTextureCollection(new List<TexturePackage>());
        }
        ...
        {
            var cordonTextureName = "tools/toolsskybox"; // todo make this configurable
        ...
        public Task<Batch> CreateBatch(IEnumerable<BatchArgument> arguments, BatchOptions options)
        {
            ...
            batch.Steps.Add(new BatchCallback(BatchStepType.ProcessBuildResults, (b, d) =>
            {
                ...
                var gameMapDir = Path.Combine(BaseDirectory, GameDirectory, "maps");
            ...
            if (options.RunGame ?? GameRun)
            {
                batch.Steps.Add(new BatchCallback(BatchStepType.RunGame, (b, d) =>
                {
                    ...
                    var gameArg = $"-game {GameDirectory} ";
